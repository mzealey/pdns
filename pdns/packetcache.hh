...
private:
  inline const string getRevQName(const string &qname)
  {
    return toLower( string(qname.rbegin(), qname.rend()) );
  };

  bool getEntryLocked(const string &revqname, const QType& qtype, CacheEntryType cet, string& entry, int zoneID=-1, 
    bool meritsRecursion=false, unsigned int maxReplyLen=512, bool dnssecOk=false, bool hasEDNS=false);
  struct CacheEntry
  {
    ...
    indexed_by <
                ordered_unique<
                      composite_key< 
                        ...
                        >,
                        composite_key_compare<std::less<std::string>, std::less<uint16_t>, std::less<uint16_t>, std::less<int>, std::less<bool>, 
